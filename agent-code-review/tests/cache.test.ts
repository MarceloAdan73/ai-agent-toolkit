...
describe('cache', () => {
  ...
  describe('loadCache', () => {
    ...
    it('loads existing cache file', () => {
      const data = { 'test.ts': { hash: 'abc123', reviewPath: 'test.review.md', timestamp: 1000 } };
      fs.writeFileSync(path.join(tmpDir, '.review-cache.json'), JSON.stringify(data));
      const cache = loadCache(tmpDir);
    ...
    it('returns empty object if JSON is corrupt', () => {
      fs.writeFileSync(path.join(tmpDir, '.review-cache.json'), '{bad json');
      const cache = loadCache(tmpDir);
      expect(cache).toEqual({});
    });
  });
  ...
  describe('saveCache', () => {
    it('writes cache file correctly', () => {
      const cache = { 'test.ts': { hash: 'def456', reviewPath: 'test.review.md', timestamp: 2000 } };
      saveCache(tmpDir, cache);
      const cachePath = path.join(tmpDir, '.review-cache.json');
  ...
  describe('isFileCached', () => {
    it('returns true if hash matches', () => {
      const cache = { 'test.ts': { hash: 'abc', reviewPath: 'test.review.md', timestamp: 1000 } };
  ...
  describe('updateCache', () => {
    it('adds an entry to the cache', () => {
      const cache = {};
      const updated = updateCache('test.ts', 'test.review.md', 'abc', cache);
      expect(updated['test.ts']).toBeDefined();
      expect(updated['test.ts']!.hash).toBe('abc');
      expect(updated['test.ts']!.reviewPath).toBe('test.review.md');
      expect(updated['test.ts']!.timestamp).toBeGreaterThan(0);
    });

    it('overwrites an existing entry', () => {
      const cache = { 'test.ts': { hash: 'old', reviewPath: 'old.review.md', timestamp: 100 } };
      updateCache('test.ts', 'new.review.md', 'newhash', cache);
      expect(cache['test.ts']!.hash).toBe('newhash');
      expect(cache['test.ts']!.reviewPath).toBe('new.review.md');
