...
import { parseReviewResults } from '../src/reviewGenerator.ts';
...
model User {
  ...
  tasks Task[]
    ...
    const mockResponse = '[{"severity":"warning","category":"Performance","line":20,"suggestion":"Add index on status field for filtering"}]';
    const reviewResults = parseReviewResults(mockResponse);
    expect(reviewResults.length).toBe(1);
    expect(reviewResults[0]!.severity).toBe('warning');

    expect(reviewResults[0]!.suggestion).toContain('index');
  });
