...
export async function generateReview(
  ...
  if (!ai) {
    console.error('[ERROR] AI model not initialized. Skipping code review.');
    return null;
  }
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      if (verbose) {
        console.log(`[INFO] Generating review (attempt ${attempt}/${MAX_RETRIES})...`);
      }
      ...
      const results = parseReviewResults(text);
      if (verbose) console.log('[INFO] Review generated successfully.');
      return results;
...
export function parseReviewResults(rawText: string | null | undefined): ReviewResult[] {
  if (!rawText) return [];
