...
import { createAIClient, generateReview, parseReviewResults } from './src/reviewGenerator.js';
...
import type { CLIOptions, ReviewResult } from './src/types.js';
...
program
  ...
  .option('--output <file>', 'Output file for review report (default: terminal)')
...
function printTerminalReport(entries: ReviewEntry[]): void {
  ...
  console.log('\n=== Code Review Report ===\n');
  console.log(`  Files reviewed: ${entries.length}`);
...
function generateMarkdownReport(entries: ReviewEntry[]): string {
  const allResults = entries.flatMap(e => e.results);
  const critical = allResults.filter(r => r.severity === 'critical');
  const warnings = allResults.filter(r => r.severity === 'warning');
  const info = allResults.filter(r => r.severity === 'info');
    ...
    try {
      ...
      if (isFileCached(filePath, cache, currentHash)) {
        ...
        if (cachedEntry?.content) {
          const cachedResults = parseReviewResults(cachedEntry.content);
          reviewEntries.push({ relativePath, filePath, results: cachedResults });
          succeeded++;
          cacheHits++;
        }
        continue;
      ...
      if (reviewResults) {
        if (dryRun) {
          const preview = reviewResults.slice(0, 3).map(r => `    ${r.severity}: ${r.suggestion}`).join('\n');
          fileSpinner.succeed(`[${i + 1}/${files.length}] ${relativePath} → [DRY-RUN] ${reviewResults.length} findings:\n${preview}`);
          succeeded++;
        } else {
          const severityIcon = reviewResults.some(r => r.severity === 'critical') ? '🔴' :
                              reviewResults.some(r => r.severity === 'warning') ? '🟡' : '🟢';
          fileSpinner.succeed(`[${i + 1}/${files.length}] ${relativePath} → ${severityIcon} ${reviewResults.length} findings`);
          reviewEntries.push({ relativePath, filePath, results: reviewResults });
          updateCache(filePath, '', currentHash, cache, JSON.stringify(reviewResults));
          succeeded++;
        }
      } else {
