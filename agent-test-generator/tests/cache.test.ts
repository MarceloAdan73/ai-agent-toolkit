...
describe('cache', () => {
  ...
  describe('loadCache', () => {
    ...
    it('loads existing cache file', () => {
      const data = { 'test.ts': { hash: 'abc123', testPath: 'test.test.ts', timestamp: 1000 } };
      fs.writeFileSync(path.join(tmpDir, '.test-cache.json'), JSON.stringify(data));
      const cache = loadCache(tmpDir);
    ...
    it('returns empty object if JSON is corrupt', () => {
      fs.writeFileSync(path.join(tmpDir, '.test-cache.json'), '{bad json');
      const cache = loadCache(tmpDir);
      expect(cache).toEqual({});
    });
  });
  ...
  describe('saveCache', () => {
    it('writes cache file correctly', () => {
      const cache = { 'test.ts': { hash: 'def456', testPath: 'test.test.ts', timestamp: 2000 } };
  ...
  describe('isFileCached', () => {
    it('returns true if hash matches', () => {
      const cache = { 'test.ts': { hash: 'abc', testPath: 'test.test.ts', timestamp: 1000 } };
  ...
  describe('updateCache', () => {
    it('adds an entry to the cache', () => {
      const cache = {};
      const updated = updateCache('test.ts', 'test.test.ts', 'abc', cache);
      expect(updated['test.ts']).toBeDefined();
      expect(updated['test.ts']!.hash).toBe('abc');
      expect(updated['test.ts']!.testPath).toBe('test.test.ts');
      expect(updated['test.ts']!.timestamp).toBeGreaterThan(0);
    });

    it('overwrites an existing entry', () => {
      const cache = { 'test.ts': { hash: 'old', testPath: 'old.test.ts', timestamp: 100 } };
      updateCache('test.ts', 'new.test.ts', 'newhash', cache);
      expect(cache['test.ts']!.hash).toBe('newhash');
      expect(cache['test.ts']!.testPath).toBe('new.test.ts');
