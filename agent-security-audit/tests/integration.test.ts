...
import os from 'os';
import { collectFiles, readFile, shouldSkipContent } from '../src/fileProcessor.ts';
import { getFileTypePrompt } from '../src/prompts.ts';
import { parseAuditResults } from '../src/securityGenerator.ts';
...
model User {
  ...
  tasks Task[]
    ...
    const mockResponse = '[{"severity":"warning","category":"Authentication","line":10,"suggestion":"Add @unique constraint on email field"}]';
    const auditResults = parseAuditResults(mockResponse);
    expect(auditResults.length).toBe(1);
    expect(auditResults[0]!.severity).toBe('warning');
    expect(auditResults[0]!.suggestion).toContain('constraint');
  });
...
describe('Cache integration', () => {
  ...
  it('detects cached vs non-cached files', () => {
    ...
    const updatedCache = updateCache(filePath, 'test.security.md', hash, {});
  ...
  it('detects file changes invalidating cache', () => {
    ...
    const cache = updateCache(filePath, 'test.security.md', hash1, {});
