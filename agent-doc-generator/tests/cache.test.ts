...
describe('cache', () => {
  ...
  describe('loadCache', () => {
    ...
    it('loads existing cache file', () => {
      const data = { 'test.ts': { hash: 'abc123', docPath: 'test.docs.md', timestamp: 1000 } };
      fs.writeFileSync(path.join(tmpDir, '.doc-cache.json'), JSON.stringify(data));
      const cache = loadCache(tmpDir);
    ...
    it('returns empty object if JSON is corrupt', () => {
      fs.writeFileSync(path.join(tmpDir, '.doc-cache.json'), '{bad json');
      const cache = loadCache(tmpDir);
      expect(cache).toEqual({});
    });
  });
  ...
  describe('saveCache', () => {
    it('writes cache file correctly', () => {
      const cache = { 'test.ts': { hash: 'def456', docPath: 'test.docs.md', timestamp: 2000 } };
      saveCache(tmpDir, cache);
      const cachePath = path.join(tmpDir, '.doc-cache.json');
  ...
  describe('isFileCached', () => {
    it('returns true if hash matches', () => {
      const cache = { 'test.ts': { hash: 'abc', docPath: 'test.docs.md', timestamp: 1000 } };
  ...
  describe('updateCache', () => {
    it('adds an entry to the cache', () => {
      const cache = {};
      const updated = updateCache('test.ts', 'test.docs.md', 'abc', cache);
      expect(updated['test.ts']).toBeDefined();
      expect(updated['test.ts']!.hash).toBe('abc');
      expect(updated['test.ts']!.docPath).toBe('test.docs.md');
      expect(updated['test.ts']!.timestamp).toBeGreaterThan(0);
    });

    it('overwrites an existing entry', () => {
      const cache = { 'test.ts': { hash: 'old', docPath: 'old.md', timestamp: 100 } };
      updateCache('test.ts', 'new.md', 'newhash', cache);
      expect(cache['test.ts']!.hash).toBe('newhash');
      expect(cache['test.ts']!.docPath).toBe('new.md');
