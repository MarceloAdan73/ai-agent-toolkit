...
export async function generatePdfFromContent(content: string, outputPath: string, title: string, agentName: string): Promise<void> {
  ...
  doc.fontSize(11).font('Helvetica').text(`Generated: ${new Date().toLocaleString('en-US')} • ${agentName}`, { align: 'center' });
  doc.moveDown(1.5);
  ...
  doc.moveDown(1);
  doc.fontSize(8).font('Helvetica').fillColor('#999').text(`Generated by ${agentName} • AI Agent Toolkit`, { align: 'center' });
  doc.end();
