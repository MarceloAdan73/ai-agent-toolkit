...
import type { RefactorSuggestion } from './types.js';
...
export async function generateRefactor(
  ...
  if (!ai) {
    console.error('[ERROR] AI model not initialized. Skipping refactor analysis.');
    return null;
  }
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      if (verbose) {
        console.log(`[INFO] Generating refactor suggestions (attempt ${attempt}/${MAX_RETRIES})...`);
      }
      ...
      const results = parseRefactorResults(text);
      if (verbose) console.log('[INFO] Refactor suggestions generated successfully.');
      return results;
...
export function parseRefactorResults(rawText: string | null | undefined): RefactorSuggestion[] {
  if (!rawText) return [];

  try {
    const jsonMatch = rawText.match(/\[[\s\S]*\]/);
    if (jsonMatch) {
      const parsed = JSON.parse(jsonMatch[0]) as RefactorSuggestion[];
