...
export function writeDocFile(
  ...
  try {
    ...
    const docFileName = `${baseName}.refactor.md`;
    ...
    const error = err as Error;
    console.error(`[ERROR] Cannot write refactor file: ${error.message}`);
    return null;
  }
}
...
export function createBackup(filePath: string): string | null {
  try {
    ...
    const error = err as Error;
    console.error(`[ERROR] Cannot create backup of ${filePath}: ${error.message}`);
    return null;
  }
}

export function applyRefactor(filePath: string, newContent: string): boolean {
  try {
    ...
    const error = err as Error;
    console.error(`[ERROR] Cannot write refactored file ${filePath}: ${error.message}`);
    return false;
  }
}
