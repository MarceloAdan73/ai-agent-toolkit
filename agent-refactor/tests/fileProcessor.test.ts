...
describe('fileProcessor', () => {
  ...
  describe('writeDocFile', () => {
    it('creates a .refactor.md file alongside the original', () => {
      const filePath = path.join(tmpDir, 'component.tsx');
      fs.writeFileSync(filePath, '<div />');
      const docPath = writeDocFile(filePath, '# Refactor Suggestions');
      expect(docPath).toBe(path.join(tmpDir, 'component.refactor.md'));
      expect(fs.existsSync(docPath!)).toBe(true);
      expect(fs.readFileSync(docPath!, 'utf-8')).toBe('# Refactor Suggestions');
    });
    ...
    it('creates file in custom output directory', () => {
      ...
      fs.writeFileSync(filePath, '<div />');
      const docPath = writeDocFile(filePath, '# Refactor', outputDir);
      expect(docPath).toBe(path.join(outputDir, 'component.refactor.md'));
      expect(fs.existsSync(docPath!)).toBe(true);
    });
  });
  ...
  describe('createBackup', () => {
    it('creates a .bak copy of the file', () => {
      const filePath = path.join(tmpDir, 'source.ts');
      fs.writeFileSync(filePath, 'export const x = 1;');
      const backupPath = createBackup(filePath);
      expect(backupPath).toBe(`${filePath}.bak`);
      expect(fs.existsSync(backupPath!)).toBe(true);
      expect(fs.readFileSync(backupPath!, 'utf-8')).toBe('export const x = 1;');
    });

    it('returns null for non-existent file', () => {
      const backupPath = createBackup(path.join(tmpDir, 'nope.ts'));
  ...
  describe('applyRefactor', () => {
    it('writes new content to the file', () => {
      const filePath = path.join(tmpDir, 'source.ts');
      fs.writeFileSync(filePath, 'export const x = 1;');
      const result = applyRefactor(filePath, 'export const x = 2;');
      expect(result).toBe(true);
      expect(fs.readFileSync(filePath, 'utf-8')).toBe('export const x = 2;');
    });

    it('returns false for non-existent file', () => {
      const result = applyRefactor(path.join(tmpDir, 'nope.ts'), 'new content');
